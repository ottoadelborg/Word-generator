...
function PlayAudio(audioUrl) {
  ...
  return (
   ...
   <section className='input--Wrapper'>
      <input onChange={ (event) => {setSearch(event.target.value)} } data-testid="searchbar"/>
      <button onClick={ WordFinder } className='search--button' data-testid="searchbutton">Search</button>
...
{/* Mappar ut allt som jag behöver från setResults state */}
            {results.map((result, index) => (
              <div key={index}>
                ...
                <p data-testid="definition">Definition: { result.meanings[0].definitions[0].definition }</p>
                ...
                <button onClick={() => PlayAudio(result.phonetics[0].audio)} className='audio--button' data-testid="audiobutton">Ljud</button>
                <hr />
              </div>
            ))}
            </section>
